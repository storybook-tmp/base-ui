...
/**
 * Stories follow research/c-components/toggle-group (Tier 3): the kept docs
 * hero (single-select) and "Multiple" demos, plus the required composite-
 * keyboard story exercising the single-tab-stop roving-focus contract. Note
 * (brief §5/§9, PR #3971): Home/End key navigation only works when a
 * ToggleGroup is used standalone — when hosted inside a Toolbar, it defers
 * entirely to the Toolbar's own CompositeRoot, which does not enable
 * Home/End (see the toolbar stories/MDX for the hosted case).
 */
const meta = {
...
export const Multiple: Story = {
  ...
  play: async ({ canvas, userEvent }) => {
    ...
    // Selection semantics: pressing a second item leaves the first one pressed too.
